...
export function Show() {
  ...
  const handleDelete = async (movieId) => {
    try {
      const response = await axios.delete(` https://bookmyshow2-cr8a.onrender.com/api/movies/deleteMovieById/${movieId}`);
...
export function AllUsers() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(' https://bookmyshow2-cr8a.onrender.com/api/users/getallUsers')
      .then((response) => {
        if (!response.ok) {
...
export function BoHis() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(" https://bookmyshow2-cr8a.onrender.com/api/bookings/getallbookings");
